...
// 상품 설정
// =============================================================
// totalStock = 2026-06-10 기준 시작 재고 (현재 remaining 반영)
//
// 실제 당첨 확률 (서버에서만 사용, 클라이언트는 모름):
//   보조배터리:    7.1%   (시각적으로는 12.5% 칸)
//   우산:          5.1%   (시각적으로는 12.5% 칸)
//   치약칫솔세트: 15.2%   (시각적으로는 12.5% 칸)
//   하리보:       41.7%   (시각적으로는 25% = 2칸)
//   비타500스틱:  30.9%   (시각적으로는 37.5% = 3칸)
//
// 설계 목표: 일반재고(하리보+비타500=571개) 소진 시점에
//           프리미엄(254개) 85% 배포 (80% 최소 보장)
// =============================================================
// reserveStock: 해당 수량 이하로는 절대 뽑히지 않는 여유 재고 (프리미엄 전용)
//   - battery:  66 × 10% ≈ 7
//   - umbrella: 47 × 10% ≈ 5
//   - toothset: 141 × 10% ≈ 15
//   - 일반상품(하리보/비타500)은 0 = 여유 없음 (끝까지 뽑힘)
const PRIZES = {
  battery:  { name: '보조배터리',   totalStock: 66,  baseProbability: 0.071, reserveStock: 7  },
  umbrella: { name: '우산',         totalStock: 47,  baseProbability: 0.051, reserveStock: 5  },
  toothset: { name: '치약칫솔세트', totalStock: 141, baseProbability: 0.152, reserveStock: 15 },
  haribo:   { name: '하리보',       totalStock: 328, baseProbability: 0.417, reserveStock: 0  },
  vita500:  { name: '비타500스틱',  totalStock: 243, baseProbability: 0.309, reserveStock: 0  },
};
...
function handleDebug() {
  ...
  try {
    ...
    if (stockSheet && allocSheet) {
      ...
      for (const key of PRIZE_ORDER) {
        ...
        const reserve = (PRIZES[key] && PRIZES[key].reserveStock) || 0;
        ...
        const canDraw = stockMap[key].remaining > reserve && effRem > 0;
      ...
      const vReserve = (PRIZES[ABSORBER] && PRIZES[ABSORBER].reserveStock) || 0;
      ...
      const vCan = stockMap[ABSORBER].remaining > vReserve && vEff > 0;
  ...
  return {
    ...
    snapshot: snapshot,
    version: 'v4-2026-06-10-3days', // ← 이 값으로 재배포 여부 확인
  };
}
...
function handleSpin() {
  ...
  try {
    ...
    // 6) 확률 계산: 재고(>reserveStock) & 할당 남아있는 상품만 확률 부여
    //    reserveStock: 프리미엄 여유 재고 — 이 수량 이하로 내려가면 더 안 뽑힘
    ...
    for (const key of PRIZE_ORDER) {
      if (key === ABSORBER) continue;
      const reserve = (PRIZES[key] && PRIZES[key].reserveStock) || 0;
      const canDraw = stockMap[key] && stockMap[key].remaining > reserve &&
    ...
    const absorberReserve = (PRIZES[ABSORBER] && PRIZES[ABSORBER].reserveStock) || 0;
    const absorberCanDraw = stockMap[ABSORBER] && stockMap[ABSORBER].remaining > absorberReserve &&
    ...
    // 8) Fallback: 확률 전부 0이거나 선택된 상품 재고 없음
    //    → 재고(>reserve)가 있는 상품 중 "stock 기반 가중 랜덤" 선택
    //    ⚠️ 과거 버그: PRIZE_ORDER 순서대로 첫 번째 선택 → battery 편향 발생
    const selectedReserve = selectedPrize ? ((PRIZES[selectedPrize] && PRIZES[selectedPrize].reserveStock) || 0) : 0;
    if (!selectedPrize || !stockMap[selectedPrize] || stockMap[selectedPrize].remaining <= selectedReserve) {
      selectedPrize = null;
      const pool = [];
      let totalWeight = 0;
      for (const key of PRIZE_ORDER) {
        const reserve = (PRIZES[key] && PRIZES[key].reserveStock) || 0;
        if (stockMap[key] && stockMap[key].remaining > reserve) {
          const w = stockMap[key].remaining - reserve; // 남은 가용 재고만큼 가중치
          pool.push({ key: key, weight: w });
          totalWeight += w;
        }
      }
    ...
    // 9) 최종: 재고 있는 상품이 정말 하나도 없음
    if (!selectedPrize) {
      return {
        error: '모든 상품 재고가 소진되었습니다.',
        stock: getCurrentStock_(stockMap),
      };
    }

    // 10) 업데이트: stock + alloc + log (순서 중요 - stock 먼저로 재고 보호)
